...
if(isset($_POST['Submit']) && $_POST['Submit']=="report_link" && $_POST['report_code']!= " " ){
	   $date = date("Y-m-d");
    $links = $_POST['link'];
     $reportexits = $functions->CheckreportexitsByEmpId($_POST['report_code'],$_SESSION['employee_id']);
    //print_r($reportexits);
    if($reportexits != []){
      ...
      'created_by'=>$_SESSION['employee_id'],'created'=>$date,'status'=>1);
      $reportId=$functions->insertReports($data);
     }
    if($reportId){
       foreach($links as $row){
      				$links = array_merge(['report_id'=>$reportId],$row,['created'=>$date]);
      				$linkId = $functions->insertLinks($links);
       }
       $result = $linkId;
    }else{
   ...
   <div class="section__content section__content--p30">
       <div class="container-fluid">
           <div class="row">
               ...
               <form action="seo_submission.php" method="post" enctype="multipart/form-data" class="form-horizontal">
                         ...
                         <div class="row form-group">
                            ...
                            <div class="col col-md-3">
                             ...
                             <div class="col col-md-2">
                               <label for="text-input" style="margin: 0!important;">D.A <i  onclick='check(1)' class="fas fa-check-circle " style="color:green;"></i></label>
                               <input type="number" name="link[1][domain_authority]"  placeholder="Only number" min="1" max="100" class="form-control domain_authority" required>
                               </div>
                             <div class="col col-md-2">
