<?php
 ...
 $reports=$functions->getSeoEmpSubmittedReportsByEmpId($_SESSION['employee_id']);
 }else{
 $reports=$functions->getAllSeoSubmittedReports();
 }
                        ...
                        <?php foreach($reports as $row){ ?>
                                <tr>
                                  ...
                                  <td><?php echo $row['created']; ?></td>
                                   <td><?php
                                    $tot=$functions->getCountOfSubmittedLinksByReportId($row['id']);
                                     echo $tot['total']; ?></td>
                                <?php if($_SESSION['role_id'] == 1){ ?>
                                   <td>
                                  ...
                                  <td><a href="view_submitted_links.php?report_id=<?php echo base64_encode($row['id']);?>"
                                  target="_blank">View</a></td>
                                 <?php }else{ ?>
                                 <td>
                                 <a href="edit_submitted_links.php?report_id=<?php echo base64_encode($row['id']);?>"
       ...
       var delete_report=$(this).val();
        // console.log(delete_report);
           $.ajax({
               type: "POST",
               url: 'ajax.php',
               data: ({delete_submitted_report:delete_report}),
