...
if(isset($_GET['leadid']) && isset($_GET['associateid'])){
	...
	if($getleaddetails['associated_id'] == "0"){
  ...
  $tomail =$functions->getToassignedmanageremail($associateId);
		...
		<tr>
		<th style="padding:5px;">Requirement</th>
		<th>:</th>
		<td>'.$getleaddetails['txt_comments'].'</td>
		</tr>
		</table>
		</div>
		</div>
...
if(isset($_POST['reason']) && $_POST['reason'] != "") {
	...
	$Insert=$accountsfunctions->insertPipeline($data);
	$leadDetails = $functions->getleadbyId($leadid);
		$to =$leadDetails['email'];
		//$to ="swapna.yarraguntla@industryarc.com";
 	$subject='Feedback Form';
 	$link = "https://feedback.industryarc.com/lsfeedback.php?auth=".base64_encode($leadDetails['email']);
 	$message=' <div class="col-sm-3">
 	<h5>Dear '.$leadDetails['fname'].' '.$leadDetails['lname'].',</h5>
 	</div>
 	<div class="col-sm-8">
 	<p><br>We sincerely appreciate your time and efforts in engaging with our team for your market
 	intelligence needs. It is unfortunate for us to not get a chance to serve your esteemed company
 	at this moment and hence would request you to allocate 60 seconds, to share your feedback on our services.</p>
 <p>
 <a href="'.$link.'" target="_blank">Click Here</a>
 </p>
 <p><br>
  Your inputs will immensely help our team to address any gaps and improve our services substantially.
  Have a great day!
 </p>
 	<div class="col-sm-3">
 	<br>Thanks,<br>
...
if(isset($_POST['report_code']) && $_POST['report_code'] != ""){
	$reportcode= $_POST['report_code'];
	$post = array('token'=>'GetReportByReportCode',
	'code' => $reportcode);
	$ch1 = curl_init();
...
if(isset($_POST['check_url']) && $_POST['check_url'] != ""){
	$check_url= $_POST['check_url'];
	$res=$functions->getdomainByUrl($check_url);
	// print_r($res);
	echo json_encode($res);
}

if(isset($_POST['delete_report']) && $_POST['delete_report'] != ""){
    $report_id = $_POST['delete_report'];
     $data = array('status'=>0);
     $result=$functions->DeleteRecordByID($report_id,$data);
    echo $result;
}

if(isset($_POST['delete_submitted_report']) && $_POST['delete_submitted_report'] != ""){
    $report_id = $_POST['delete_submitted_report'];
     $data = array('status'=>0);
     $result=$functions->DeleteSubmittedRecordByID($report_id,$data);
    echo $result;
}
