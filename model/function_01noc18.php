 ...
 /*------------------------- call logs functions ----------------------------------------------*/
	...
	function getAllleadsHaveFollowupDates(){
		if($this->leadGenChannel==""){
			...
			$roleQuery = "SELECT * FROM leads WHERE next_followup_date<>'0000-00-00 00:00:00' AND next_followup_date IS NOT NULL
 AND `status`=1 AND lead_stage_id IN (1,2,5) ORDER BY id DESC";
		}else{
			$roleQuery = "SELECT * FROM leads WHERE lead_generation_channel_id in ($this->leadGenChannel) and next_followup_date<>'0000-00-00 00:00:00' AND next_followup_date IS NOT NULL AND `status`=1 AND lead_stage_id IN (1,2,5) order by id desc";
		}
		$result = $this->dbObject->getAllRows($roleQuery);
		return $result;
	}
