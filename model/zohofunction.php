...
class zohofunctions {
    ...
    function __construct()
    {
        ...
        $this->authToken = $this->generateAccessToken($_SESSION['zoho_ref_token']);
    ...
    function generateAccessToken($refToken){
        $curl = curl_init();
        curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://accounts.zoho.com/oauth/v2/token?refresh_token='.$refToken.'&client_id=1000.XLPQ2G9GFRJNDF7V8LCFS56Y24PVJC&client_secret=89dac74ab85a58803068afe725469b358e641d2613&redirect_uri=https://crm.industryarc.in/api/zoho/authtoken.php&grant_type=refresh_token',
    ...
    function getCurrencyList(){
        ...
        $urlAuthKeys = "?organization_id=".$this->organizationId;
    ...
    function createZohoContact($arrInputs=[]){
        ...
        if(isset($arrInputs['contact_details']) && !empty($arrInputs['contact_details'])){
            ...
            $input = json_encode($arrInputs['contact_details']);
            $curl = curl_init();
    ...
    function createZohoItems($arrInputs,$otherParams){
        ...
        if(!empty($arrInputs['item_details']) && empty($itemDetails)){
            ...
            $input = json_encode($arrInputs['item_details']);
       ...
       // echo "<pre>";
        ...
        if(isset($arrResponce['code']) && $arrResponce['code']==0){
            $arrEmailParams=[
                ...
                'cc_mail_ids'=>['mkt@industryarc.com',$_SESSION['email'],'accounts@industryarc.com'],
                'invoice_number'=>$arrResponce['invoice']['invoice_number'],
                'invoice_id'=>$arrResponce['invoice']['invoice_id'],
    ...
    function emailInovice($arrEmailParams=[]){
        ...
        if(isset($apiInputData) && !empty($apiInputData)){
            ...
            $input = json_encode($apiInputData);
